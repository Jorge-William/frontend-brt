...
const onboardingSchema = z.object( {
  ...
  phone: z
    .string()
    .min( 15, { message: "Telefone é obrigatório" } )
    .regex( /^\([0-9]{3}\) [0-9]{4,5}-[0-9]{4}$/, {
...
const CriarContaPage = observer( () => {
  const userStore = OnboardingStore; // Add store reference
  ...
  const onSubmit: SubmitHandler<OnboardingFormValues> = async ( data ) => {
    ...
    try {
      ...
      const userData = {
        ...
        firstName: data.fullName.split( " " )[ 0 ].trim(),
        lastName: data.fullName.split( " " ).slice( 1 ).join( " " ).trim(),
        password: data.password,
        cellphone: data.phone.replace( /\D/g, "" ), // Remove non-digits
      };

      console.log( "Submitting user data:", userData ); // Log the payload
      ...
      navigate( "/onboarding/verificar-codigo" );
    } catch ( error ) {
        ...
        <div className="space-y-8">
          <div className="space-y-3">
            ...
            <p className="text-gray-600">
              Controle agendamentos, clientes e serviços em uma única
              plataforma.
            </p>
          </div>
          ...
          <div className="space-y-3">
            ...
            <p className="text-gray-600">
              Automatize processos e reduza faltas com lembretes automáticos.
            </p>
          </div>
          ...
          <div className="space-y-3">
            ...
            <p className="text-gray-600">
              Ofereça uma experiência moderna de agendamento online 24/7.
            </p>
          </div>
        </div>
      </div>
      ...
      <Card className="w-full max-w-md shadow-xl z-10 bg-white/80 backdrop-blur-md lg:ml-8">
        ...
        <CardContent>
          <form onSubmit={handleSubmit( onSubmit )}>
            {" "}
            {/* Usa o handleSubmit do react-hook-form */}
            <div className="grid w-full items-center gap-4">
              {" "}
              {/* Reduzido o gap para melhor densidade */}
              {/* Campo Nome Completo */}
              ...
              <div className="flex flex-col space-y-1.5">
                ...
                <div className="flex space-x-1 h-1.5 mt-1">
                  <div
                    className={`flex-1 rounded-full transition-colors duration-300 ${passwordStrength >= 1 ? getStrengthColor( 1 ) : "bg-gray-200"}`}
                  ></div>
                  <div
                    className={`flex-1 rounded-full transition-colors duration-300 ${passwordStrength >= 2 ? getStrengthColor( 2 ) : "bg-gray-200"}`}
                  ></div>
                  <div
                    className={`flex-1 rounded-full transition-colors duration-300 ${passwordStrength >= 3 ? getStrengthColor( 3 ) : "bg-gray-200"}`}
                  ></div>
                  <div
                    className={`flex-1 rounded-full transition-colors duration-300 ${passwordStrength >= 4 ? getStrengthColor( 4 ) : "bg-gray-200"}`}
                  ></div>
                </div>
                {errors.password ? (
                  <p className="text-xs text-red-600">
                  ...
                  <p className="text-xs text-muted-foreground mt-1">
                    Use 8+ caracteres com maiúsculas, minúsculas, números e
                    símbolos.
                  </p>
                )}
              </div>
